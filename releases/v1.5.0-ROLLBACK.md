# Rollback plan: v1.5.0 -> v1.4.0

## When to use this
The `v1.5.0` deployment is misbehaving (elevated error rate, failed health checks,
...
az containerapp update \
  ...
  --image globantfulfilmentacr.azurecr.io/agentic-fulfilment-app:1.4.0
```
This points the Container App back at the exact image that was running before
`v1.5.0` was deployed — no rebuild needed, the image is already in the registry.
...
(`git diff v1.4.0..v1.5.0 -- backend/db.py`) and confirm
...
## 4. Follow-up
File an issue capturing what went wrong in `v1.5.0` before re-attempting the release.
