# Rollback plan: v1.3.2 -> v1.3.1

## When to use this
The `v1.3.2` deployment is misbehaving (elevated error rate, failed health checks,
...
az containerapp update \
  ...
  --image globantfulfilmentacr.azurecr.io/agentic-fulfilment-app:1.3.1
```
This points the Container App back at the exact image that was running before
`v1.3.2` was deployed — no rebuild needed, the image is already in the registry.
...
## 4. Follow-up
File an issue capturing what went wrong in `v1.3.2` before re-attempting the release.
